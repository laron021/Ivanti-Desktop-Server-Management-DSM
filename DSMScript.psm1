...
#>
#
# This program executes software rollouts and delete policies in DSM according to the given parameters.
#
# Program can get executed in Powershell ISE. Remember to modify the softwareID-s, groupID-s
# Destination folder for the log file: D:\Work\Scripts
# ------------------------------------------------------
# Import-Module "D:\Work\arlakato\DSMScript.psm1" -Force
# -------------------------------------------------------
...
function regex {
    ...
    $regex = $m -match '[0-2][\d][:][\d][\d][\s][0-2][\d][.][0-3][0-9][.][2][0][2][3]'
    if (!$regex){
...
function totalcleanup {
    ...
    #DENY,STANDARD
    $remove_from = @(440274,440275,440276,440271,440272,440273,440792,440793,440794) #RG1-RG2-RG3-QSA-PILOT
...
function welle {
    ...
    foreach ($v in $wpolicy ) {  
               ...
               if ($v.Schematag -eq "DenyPolicy") {
                    ...
                    if ($targetwelle -eq 160659){

                        $x = $v.AssignedObjectName
                        $y = Get-DSM7PolicyListByTarget -ID 160659 | select ID,AssignedObjectName | where { $_.AssignedObjectName -match $x}  
...
function indiprod {
    ...
    $value = $_.value #DL-SWD
        ...
        $list = Get-DSM7PolicyListByTarget -ID $key | select ID,SchemaTag,AssignedObjectName | where { $_.ID -notin $exc }
        foreach ($x in $list) {
                ...
                elseif($x.ID -in $revupdate){
                    ...
                    #CURRENTLY SET FOR C45!!
                    Copy-DSM7Policy -ID $x.ID -TargetID $value -IsActiv -ActivationStartDate $start |timestamp #-IsUserPolicyAllassociatedComputer
